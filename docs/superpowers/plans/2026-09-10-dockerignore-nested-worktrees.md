...
- No other doc references the current line-by-line contents of `.dockerignore` as a live claim (`grep -rln dockerignore docs/` hits only past plan files, which are historical records, not live specs — nothing to correct there per "Plans are records, not specs").
