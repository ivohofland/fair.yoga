...
Expected: the FIRST new test (NOT_FOUND) PASSES already — it pins existing behavior (`deleteStudentAccount`'s pre-existing Invitation anonymization), not new functionality, so it is not part of this task's RED step. The SECOND new test (the mid-transaction race) FAILS, but not at `waitUntilBlockedBy`: before the fix, nothing in `acceptInvitation`'s ungated transaction conflicts with the erasure's held `FOR NO KEY UPDATE` — `linkTeacherStudent`'s own child-row insert takes only `FOR KEY SHARE`, which conflicts with neither gate mode (`db-locks.ts`'s own docblock for `lockStudentForErasure` states this explicitly) — so the insert proceeds unblocked and `waitUntilBlockedBy` itself fails with "nothing waited behind backend N within 1500ms", before the test ever reaches its `STUDENT_ERASED` assertion.
