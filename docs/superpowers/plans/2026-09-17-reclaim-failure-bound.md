...
**Architecture:** Add a dedicated `failedReclaimAttempts` counter, bounded by a new `ACQUIRE_LOCK_MAX_RECLAIM_FAILURES` constant, incremented only when `reclaimStaleLock` returns `false`. No reset logic is needed or added — see "Why no reset" below. On hitting the bound, throw a distinct error naming the reclaim itself as broken, so this failure mode doesn't read as either of #631's two messages (a generic "Timed out waiting for lock" or "lock info stayed unreadable for N consecutive passes").
...
**Why no reset is needed (read before implementing):** Entry into the `if (stale && !reclaimedStale)` branch requires `!reclaimedStale`, and `reclaimedStale` only ever transitions `false → true`, never back. So any run of consecutive entries into this branch is, by construction, an unbroken run of *failures* starting from the very first entry — the moment a reclaim succeeds, `reclaimedStale` flips permanently and the branch becomes unreachable for the rest of this `acquireLock` call. There is no code path in which `failedReclaimAttempts` accumulates across two *different* failure episodes within one call, so no reset is required (see #631's `unknownPasses`, which genuinely needs resets, for contrast — that counter is read across branches that stay reachable after a reset, this one is not). Do not add speculative reset logic for this counter; it cannot be exercised by any reachable, verifiable test, and would violate CLAUDE.md's "don't add validation for scenarios that can't happen."
