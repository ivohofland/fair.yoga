...
- [ ] **Step 4: Rewrite retention.** In hits 24–27 build `scopeSweep(prisma, { WaitlistEntry: { classId: { in: [<fixture class ids>] } } })` and pass `.db` to `reapClosedWaitlistEntries`. `Class` is not scoped: the reap issues no Prisma `class.*` statement. The `class.count` near its second `groupBy` sits inside a comment, which is where this plan's first draft wrongly read it from. Hit 26's `classes >= 2` becomes exact. Hit 23 is **unchanged**: its bracket `before - after === summary.deleted` is the right shape. Do not scope it, or the bracket stops measuring the whole table.
