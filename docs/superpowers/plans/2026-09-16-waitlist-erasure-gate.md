...
3. A new section, *The `Student` row is the erasure's gate (#183)*, placed after *Ordering BETWEEN `StudioClass` and its `CalendarEntry`*. Content:
   ...
   - **why gating one writer at a time is safe**: an ungated writer takes only `FOR KEY SHARE` on the row — **provided no transaction that holds a `Class` row or a `KEY SHARE` on the student also UPDATES the `Student` row** (an `UPDATE` takes `FOR NO KEY UPDATE`, which the erasure's opening lock blocks). State that proviso as the rule, and that `POST /api/registrations` writes `tierSelectedAt` after its transaction commits for exactly this reason (it used to write it inside, which closed two cycles with the erasure — spec §1, *Correction*). Give the re-derivation command for "who updates a `Student` row": `git grep -n -E '(tx|db|prisma)\.student\.(update|updateMany|upsert)' -- src ':!*.test.ts'`, then read each hit's enclosing transaction;
