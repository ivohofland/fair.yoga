...
| `src/services/class-transitions.ts:257-270` (was `:200-211`) | Two corrections in one block. The PUT "is not a normal thing to do" is **wrong** — check-in renders on an `open` class within 15 minutes of start, so it is the designed flow — and the gap is now **closed** by Task 6's source-status scope. Rewrite to state what actually protected it (the UI only writes within the counted set) and what protects it now. Anchor phrase if the number has moved again: `not a normal thing to do` (currently `:265`). |
| `src/services/class-transitions.ts:310-312` (was `:253-258`) | "The identical stale-window race is still live in `autoTransitionToInProgress` above and `autoCompleteClasses` below" — both fixed in Tasks 4 and 5. Delete or rewrite to point at them. Anchor phrase: `The identical stale-window race is still live in` (currently `:310`). |
| `src/services/class-transitions.ts:247+` (was `:218-230`) | The enumeration of `WaitlistEntry` writers gains `closeQueueOnStart`. Count and re-state. Anchor phrase: `Two writers are outside that` (currently `:247`) — and note the enumeration's own count word is part of what changes. |
| `src/services/waitlist.ts:721-723` (was `:722-724`) | Narrow, do not delete: the PUT now guards **current registration status** and **class status**, and still does not guard **class time**, deliberately. **It cites `class-transitions.ts:199` by line number and this branch moved that block** — the prediction held: there is no such comment at `:199` any more, and the text it meant is now at `:247`/`:265`. **Drop the line number rather than re-deriving it** — this is the second time a citation in this file has gone stale, and naming the comment instead of its address is what stops a third. |

**These four references were re-derived at `e1fd1d0`**, after Tasks 4 and 5 added ~80 lines above them in `class-transitions.ts`. Tasks 7 and 8 touch neither `class-transitions.ts` nor `waitlist.ts`, so they should still be correct when this task runs — but each row carries an anchor phrase, so verify by `grep` for the phrase rather than trusting the number.
