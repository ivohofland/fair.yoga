...
- **Class detail** — one adaptive page by lifecycle: badge + meta + progress always; then draft (pricing preview + publish), open (students + estimate), check-in (attendance checklist: large names, 44px teal check tiles; Finish class from 15 min before the end, with an inline confirm), completed (attendance read-only behind "Edit attendance", earnings in a teal-tint card, Display-size tabular number, transparent breakdown + payment checklist), cancelled (quiet notice).
