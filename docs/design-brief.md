...
- **Schedule (home).** Chronological card list of the week — not a calendar grid. Each class card: day/time label + StatusBadge, `type-subtitle` class name + chevron, room caption, RegistrationProgress. Studio classes inline but lighter (dashed border, no bar). Past dimmed 40%, cancelled struck through.
