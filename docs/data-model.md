...
| delivered | boolean, default true | Whether this invitation was safe to disclose to its invitee the last time one of its writers touched this column — not every write to this row does. The writers: `inviteContact`'s create and revive paths (both `false` when a `TeacherBlock` blocks the address or the pair is already linked, re-derived fresh each time rather than fixed at the row's first write), and `PUT /api/invitations/[id]` on any `email` change, unconditionally to `false` (no delivery attempt has been made to a newly-written address, full stop). Every other writer of this row leaves it as it stood. A never-delivered row is the one a decoy invitation produces (#417/#418's gate) — the marker exists so a later writer (`unlinkTeacher`'s tombstone) can scope a mutation to rows that were actually delivered, without re-deriving delivery status from current `TeacherBlock`/roster state. Every row that existed when this column's migration ran backfilled to `true` regardless of its actual delivery history, so a decoy planted before that migration stays indistinguishable from a genuine delivery until one of the writers above next touches its row |
| **Timestamps** | | |
| created_at | datetime | |
| **Constraints** | | |
