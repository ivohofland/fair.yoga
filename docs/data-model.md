...
- **Status indexes are plain, never partial** (#224). `WaitlistEntry_status_idx` and `Class_status_idx` are ordinary btree indexes on `status`, serving the every-minute sweeps: `reconcileWaitlists`'s opening `groupBy` (`services/waitlist-reconciliation.ts`) and `autoCompleteClasses` (`services/class-transitions.ts`). A partial index (`WHERE status = 'waiting'`) looks like the better shape and serves nothing: Prisma binds every enum value as `CAST($1::text AS "WaitlistStatus")`, Postgres never folds that cast to a constant, and so the planner cannot prove a partial predicate and will not use the index — even with `enable_seqscan = off`, where the same query written with a literal uses it. `WaitlistEntry_waiting_position_key` is enforcement, not a read path, for the same reason. Measured 2026-09-24 on synthetic data (6 classes/week/teacher, 5% open): at 500 teachers × 3 years the reconciliation `groupBy` went from 25.3 ms to 14.0 ms with the `WaitlistEntry` index alone and to 25.5 ms with both (the `Class` index tempts the planner into a hash join over every open row), while `autoCompleteClasses` went from 40.5 ms to 0.03 ms — both kept for the net saving. What the join still costs, and no status index removes, is one `Class` and one `CalendarEntry` key probe per waiting entry. `src/services/status-indexes.test.ts` pins both definitions exactly, which is what refuses a narrowing to a partial index.
