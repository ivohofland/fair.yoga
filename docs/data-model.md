...
- **Terminal status is the second, wider freeze** (#247). Once a Class is `completed` or `cancelled`, `updateClass` refuses every field edit — the class, not a column list — and `PUT /api/classes/[id]` answers 409; unlike settings_locked it never lifts. `date` is additionally frozen in the database by the `class_terminal_date_guard` trigger, because the waitlist retention sweep above deletes on a status-plus-date predicate and reads that column before it does. The trigger is narrower than the service on purpose: it guards `date` alone, so the two layers are not the same rule twice.
