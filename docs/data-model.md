...
- **Retain (current behaviour).** The block keeps working, and it is not inert after erasure: the person's real mailbox still exists in the world, so if the teacher re-types that address `inviteContact` computes `delivered: false` and no invitation email is sent. Retention is the only thing standing between an erased person and mail from the one teacher they explicitly refused. The cost is a retained plaintext address for someone who asked to be forgotten — on a row they can no longer reach to clear, since erasure destroys the student profile the unlink UI hangs off. (Not because the account is gone: `deleteStudentAccount` rewrites the account email and drops sessions only when no live teacher profile still uses that account.)
...
**A plain decline's refusal does not survive that anonymisation, and #522 tracks it as a defect rather than a decision.** `declineInvitation` writes no `TeacherBlock` — `unlinkTeacher` is this repo's only writer of one — so for a plain decline the declined row's own `(teacher_id, email)` key *is* the refusal, and `inviteContact` looks it up by the address the teacher types. The scrub above moves that key, so a student who declined a teacher and then erased can be re-invited by that teacher, and `deliverInvitation` mails their real address. The unlink path is unaffected: it always writes a `TeacherBlock`, and those are retained. Recorded here because it gives the trade-off below a second side — scrubbing `Invitation.email` shields the subject from the teacher's gaze at the cost of their own standing refusal.
