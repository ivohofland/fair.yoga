...
- The **`declined`** half stays unconditional because narrowing it would strand the student. `unlinkTeacher` writes the tombstone and deletes the link in one transaction, so `declined` implies unlinked at the moment it is written, and every ordinary route back creates the link anyway. Where it does not — a promotion re-linking around a raced unlink — a narrowed rule would leave that student linked, unblocked and permanently un-re-invitable behind a tombstone `DELETE /api/invitations/[id]` refuses to remove. Reversing a decline is the escape hatch the whole decline design rests on: permanent from the teacher's side, always reversible from the student's.

The fact that tells the two apart is read off the roster link's own write, not from a column on this table. `linkTeacherStudent` is a single `INSERT … ON CONFLICT DO NOTHING` and reports whether it inserted, so the reading cannot race a concurrent writer the way a `findUnique` on either side of it could. A stored origin marker would also answer a subtly different question — where a row came *from*, when what matters at resolution time is what the pair is *now* — and `PUT /api/invitations/[id]` can move a row's `email` onto a linked pair, which is exactly where those two come apart.
