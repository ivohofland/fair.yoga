...
3. The class, under its locks: `lockLiveStudent` (409 `STUDENT_ERASED`), then `lockClassRow`; a missing class 404; another teacher's class 403; a cancelled one `CLASS_CANCELLED`; outside the window `WALK_IN_WINDOW_CLOSED`; then the already-registered answer, `200 unchanged`; then `CLASS_NOT_BOOKABLE`. Capacity never refuses a walk-in. The window refusal precedes the unchanged answer because it makes the goal moot — outside the window the request is not a walk-in at all — so an address already booked in the class is refused there like any other and answers nothing about who it is.
4. After `Registration`, `completeWalkIn`: the privacy seed (create branch), the roster link, the invitation insert and its compare-and-set on `status: 'pending'` — a miss is re-read: `accepted` is what was asked for, `declined` (a decline that landed after step 2) → `DECLINED`, gone → `NOT_FOUND` — then the notification, then a `TeacherBlock` re-read as the last statement, the #537 pattern `acceptInvitation` uses. A block committed after step 2's read (an unlink of an undelivered row leaves its invitation `pending` beside the block) → `WALK_IN_REFUSED`.
...
4. **An account whose live student profile is at another address never claims the walk-in's row.** An address whose `Account` already holds a live `Student` at a different email gets an unclaimed walk-in `Student` that sign-in never claims: `resolveOrClaimAccount` (`src/lib/auth/account.ts`) returns early when an `Account` exists, and one live profile per account (#623) rules out attaching it. The person is reachable by email — the fallback reads `Student.email` — but not through their signed-in inbox.
5. **In the window, an already-booked address answers silently.** A correct guess of the address of a student already registered in the class answers `200 unchanged` and writes nothing; a wrong guess is loud, since it registers and emails the address holder. Outside the window the walk-in refuses before the unchanged check (step 3), so no silent answer exists there.
