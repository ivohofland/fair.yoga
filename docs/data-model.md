...
teacher link. And `completeWalkIn` (`src/services/walk-ins.ts`), on a walk-in
that created the `Student` row: it seeds `share_full_name` and `share_email`
true for the walk-in's teacher and leaves every other flag at its default.
The teacher typed that name and that address at the door, so sharing them
back discloses nothing the teacher did not supply; they have no claim on the
phone, birthday or home address. A walk-in that found an existing `Student`
seeds nothing — that person's own settings, or the absence of a row, govern.
Neither the second write nor the third is an opt-in: the second is the system
silencing every share on the student's behalf because deleting the link alone
does not stop the teacher reaching them, and the third is the teacher's own
input handed back to them. Until one of those sites has run there is no row,
and every read treats absence as maximum privacy (`privacy?.shareX ??
false`). One projection reads these flags for nearly every teacher-facing
surface: `src/lib/student-visibility.ts`. The one exception is
`rosterLinkState` (`src/services/invitations.ts`), which decides what
`POST /api/students` may tell a teacher about a guessed address (#412)
outside that projection, and answers "may this teacher see this address"
the way the projection does: this teacher's `StudentPrivacy.shareEmail`,
...
```sh
grep -rnE "studentPrivacy\.(create|createMany|upsert|update|updateMany)\(" \
  --include="*.ts" --include="*.tsx" src | grep -v '\.test\.ts'
```

The match is syntactic: a nested write inside a `Student` write would escape
it, which is what this second command, printing nothing today, checks:

```sh
grep -rnE "studentPrivacy: *\{ *(create|upsert|update|connectOrCreate)" \
  --include="*.ts" --include="*.tsx" src | grep -v '\.test\.ts'
```
...
Since #412, a live link is no longer sufficient on its own to refuse the invite: `inviteContact` refuses (`ALREADY_LINKED`) only when the link exists AND the teacher could already have had that address anyway — which is two disjuncts: the student's `StudentPrivacy.shareEmail` for this teacher is true (claimed or not — see the StudentPrivacy section above), or this same `accepted` status holds. See `rosterLinkState`, `src/services/invitations.ts`. A linked student who has not shared their email gets a real, silent pending invitation instead of a refusal that would confirm the address belongs to one of this teacher's own students.

An unclaimed linked student needs no disjunct of its own. The only creator of one is a walk-in, and a walk-in leaves this teacher's invitation for the address `accepted` on either branch — the second disjunct — and on the create branch seeds `shareEmail` too, the first. Without either, such a contact would fall through to an invitation that could never arrive: `notifyInvitee` returns early on a live link, so the row would sit in Contacts under a name the teacher typed, undeliverable and unexplained.
...
**Ten lines today**: the two function definitions, plus every production site
that creates a roster link or resolves an invitation on one. Four link
...
'pending'`, because the accept IS the answer. `completeWalkIn`
(`src/services/walk-ins.ts`) resolves by its own hand too, with its own
compare-and-set on `status: 'pending'`, like `acceptInvitation` — the walk-in
is the answer (Walk-ins, below) — and never through `resolveInvitationOnLink`,
which would lift a block on a teacher's act. So a link creator turning up here
...
1. **Account existence is readable after a walk-in.** `claimedAt`, the projected name, the seeded flags and the tier distinguish an address that had a `Student` from one that did not. Accepted because every probe is loud: it registers the address holder in a real class, emails them naming the teacher and the class, creates a payment obligation they will see, only works inside a live class window, and — on the new-contact path — spends the teacher's student-write budget. #166 closed a *silent* oracle; this one announces itself to its subject.
2. **A block refusal is observable.** An invite to a blocked address answers identically to any other; a walk-in refuses. The alternative is re-linking the person who unlinked to get away from that teacher. The copy names no cause.
3. **A #412 decoy is walkable.** A pending invitation a teacher created for a guessed address of an already-linked student (`delivered: false`) is listed and walkable; walking it in registers that roster student and so confirms the guess. The picker cannot omit it without revealing `delivered`, which is the same oracle. Loud, as in 1.
4. **The attach guard is defensive.** A created row is attached only to an `Account` with no live student profile (`account.students` has no live row, `src/services/walk-ins.ts`; one live profile per account, #623). The state it defends against — an `Account` at the walk-in's address holding a live `Student` at a different address — is unreachable while a live `Student`'s email mirrors its account's address: every write that links a `Student` to an `Account` (`resolveOrClaimAccount`, `api/account/student-profile`, the walk-in's own attach) links one at the account's own address, a live `Student`'s email is never edited (it is not in `updateStudentSchema`; erasure rewrites it only in the write that ends the row's liveness), and `Account.email` changes only to an erasure placeholder (`src/services/gdpr.ts`), written once no live profile remains on the account. If that ever changes, the row the walk-in leaves there is unclaimed and sign-in would not claim it: `resolveOrClaimAccount` (`src/lib/auth/account.ts`) returns early when an `Account` exists.
5. **In a bookable class, in the window, an already-booked address answers silently.** A correct guess of the address of a student already registered in the class answers `200 unchanged` and writes nothing; a wrong guess is loud, since it registers and emails the address holder. That silent answer exists only while the class is `open` or `in_progress` and inside the window. Outside that, every walk-in refusal precedes the unchanged check (step 3), so no silent answer exists there.
...
- **Teacher timezones are stored under their current IANA name** (#258). V8 enumerates CLDR's identifiers, which never change once published; IANA renames. So `Intl.supportedValuesOf('timeZone')` and a browser's detected zone can spell one zone two ways (`Europe/Kiev`, `Europe/Kyiv`). Every schema that accepts the column — signup's detected zone (`POST /api/account/teacher-profile`) and the Settings save (`PUT /api/teachers/[id]`); re-derive with `grep -n "defaultTimezone:" src/lib/schemas.ts` — passes the value through `modernTimeZone` (`src/lib/iana-timezone.ts`), so the column and the Settings picker agree on one spelling for every zone the picker lists. A valid alias outside those renames (`CET`, `US/Eastern`) is stored as given and shown as its own option. The pairs are the renames, **not** tzdata's link targets: IANA also links a zone to another country's when their clocks have agreed since 1970, and following those would file Asmara under Nairobi, Pohnpei under Guadalcanal, Chuuk under Port Moresby and Atikokan under Panama. Measured 2026-09-26, Node v22.22.2 against tzdata `2026c`: V8's 418 zones and `zone.tab`'s 418 differ in 19 names each way, one-for-one, and those 19 are `MODERN_ZONE_NAMES`. The command below re-derives both name sets; that each pair names one zone is pinned by `src/lib/iana-timezone.test.ts`, which asserts `Intl` resolves both sides alike. Re-derive:
...
- **`Class` and `ClassTemplate` each carry six economic `CHECK` constraints** (#221, `20260924190000_class_economics_checks`) — `_room_cost_check` (`roomCost >= 0`), `_min_students_range_check` (`minStudents BETWEEN 0 AND 200`), `_max_students_range_check` (`maxStudents BETWEEN 1 AND 200`), `_students_order_check` (`minStudents <= maxStudents`), `_rate_order_check` (`minRate <= targetRate`), `_room_subsidy_check` (`minRate >= -roomCost`). Re-derive rather than trusting this list: `grep -n "CHECK" prisma/migrations/*_class_economics_checks/migration.sql`.
  - **The `minStudents` floor is 0 in the database, 1 in Zod.** Zero breaks no arithmetic — `calculateEffectiveTeacherRate`'s clamps return early whenever `minStudents == maxStudents` and for every count once `minStudents > maxStudents`, so a zero minimum is never a divide-by-zero. "At least one student" is a product rule, not a data-integrity one, and Zod (`src/lib/schemas.ts`) keeps enforcing it on every request body; the database only rules out rows that are impossible outright. The gap between the two floors is not incidental: test fixtures that must stay clear of the live auto-cancel sweep write `minStudents: 0` on purpose, to keep a class open under it (`AutoCancelCheck` has no "never" value) — a database floor of 1 would have broken every one of them. Re-derive the roster rather than trusting a name list:
    ```bash
    grep -rn "minStudents: 0" --include="*.test.ts" src tests \
      | grep -v 'class-economics-constraints.test.ts' \
      | grep -vE '^[^:]*:[0-9]+:[[:space:]]*(//|\*)' \
      | cut -d: -f1 | sort -u
    ```
