...
- **The row is already the minimum, and the teacher never sees it.** It holds the teacher, the address a refusal has to be matched on, and when it was written. The retained address is never surfaced to the teacher (see the *Why the two resolve in opposite directions* paragraph below), so nobody the subject refused can read it back.
