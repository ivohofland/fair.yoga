...
import { acceptInvitation, inviteContact, notifyInvitee, unlinkTeacher } from '@/services/invitations';
...
describe('Invitation and TeacherStudent take one lock order (#174 task 7)', () => {
  ...
  /**
   * A fresh teacher/student pair in the exact interleaved state #174's third
   * gap needs: the roster link already exists — as if the student booked a
   * class — and a SEPARATE Invitation row for the same address is still
   * pending — as if the teacher added them as a CRM contact independently.
   * Both preconditions are real; see the docblock on the test below.
   *
   ...
   */
  async function makeLinkedStudentWithPendingInvite() {
  ...
  /**
   * The real regression test: the actual `acceptInvitation` (now taking
   * `TeacherStudent` before `Invitation`, matching `unlinkTeacher`) racing a
   * real-shaped unlink. Whichever of the two Postgres happens to run first,
   * this must never reject.
   */
  it('does not deadlock when a real accept races an unlink', async () => {
    const { teacherId, studentId, email, invitationId } = await makeLinkedStudentWithPendingInvite();
