...
import { ToggleStudioTemplateButton } from './toggle-studio-template-button';
 ...
 */
describe('ToggleStudioTemplateButton', () => {
  ...
  it('sends state=active when the template is not active', async () => {
    ...
    await waitFor(() =>
      expect(fetchMock).toHaveBeenCalledWith('/api/studio-class-templates/tpl-1?state=active', {
        method: 'PATCH',
      }),
    );
  });
  ...
  it('sends state=paused when the template is active', async () => {
    stubFetch(pausedOk);
    render(<ToggleStudioTemplateButton templateId="tpl-1" isActive={true} />);

    fireEvent.click(screen.getByRole('button'));

    await waitFor(() =>
      expect(fetchMock).toHaveBeenCalledWith('/api/studio-class-templates/tpl-1?state=paused', {
        method: 'PATCH',
      }),
    );
  });
  ...
  it('does not report a network error when the update succeeds but the body cannot be read', async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      '[toggle-studio-template] updated, but response body was unreadable',
  ...
  it('reports a network error when fetch itself throws', async () => {
    ...
    expect(screen.getByRole('button')).toBeEnabled();
    expect(consoleError).toHaveBeenCalledWith(
      '[toggle-studio-template] request failed',
      expect.objectContaining({ templateId: 'tpl-1' }),
    );
  });
});
