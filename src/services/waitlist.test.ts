...
describe('withdrawWaitingEntriesForTeacher re-checks status against a promotion committed mid-wait (#241)', () => {
  // Own fixture and suffix, so the #453 and #241-withdraw describes above see
  // none of these rows.
  const scopeSuffix = `wl-race-${Date.now()}-${crypto.randomBytes(3).toString('hex')}`;
  let teacherId: string;
  let teacherAccountId: string;
  let roomId: string;
  let studentSId: string;
  ...
  beforeAll(async () => {
    ...
    const classC = await createClassFixture(prisma, {
      teacherId,
      teacherRoomId: teacherRoom.id,
      classType: 'Race scope class',
      date: new Date('2099-09-01'),
      startTime: hhmmToTime('09:00'),
      durationMinutes: 60,
      roomCost: 20,
    ...
    await prisma.waitlistEntry.create({
      data: { classId: classCId, studentId: studentSId, position: 1, status: 'waiting' },
    });
  });

  afterAll(async () => {
    await prisma.notification.deleteMany({ where: { recipientId: studentSId } });
    await prisma.waitlistEntry.deleteMany({ where: { studentId: studentSId } });
    await prisma.teacherBlock.deleteMany({ where: { teacherId } });
    await prisma.studentPrivacy.deleteMany({ where: { studentId: studentSId } });
    await prisma.teacherStudent.deleteMany({ where: { studentId: studentSId } });
    await prisma.calendarEntry.deleteMany({ where: { teacherId } });
    await prisma.student.deleteMany({ where: { id: studentSId } });
    await prisma.teacherRoom.deleteMany({ where: { teacherId } });
    await prisma.room.deleteMany({ where: { id: roomId } });
    await prisma.teacher.deleteMany({ where: { id: teacherId } });
    await prisma.account.deleteMany({
      where: { id: { in: [teacherAccountId, studentSAccountId] } },
    });
    await prisma.$disconnect();
  });

  // Copied per file rather than shared, matching the #453 describe above —
  // each site's fixture is independent.
  const captureLockSets = (): string[][] => {
    const original = dbLocks.lockClassRowsOrdered;
    const lockSets: string[][] = [];
    const spy = vi.spyOn(dbLocks, 'lockClassRowsOrdered').mockImplementation(async (tx, source) => {
   ...
   * its class still lands in the ids handed back — so `S`'s entry surviving
   * as `promoted` here is entirely down to the subsequent `updateMany`
  ...
  it('leaves a promoted entry alone when the promotion committed while the withdrawal waited', async () => {
    ...
    const holder = prisma.$transaction(
      async (tx) => {
        await tx.$queryRaw`SELECT id FROM "Class" WHERE id = ${classCId} FOR UPDATE`;
        await tx.waitlistEntry.update({
          where: { classId_studentId: { classId: classCId, studentId: studentSId } },
    ...
    // The withdrawal only returns once the holder's `FOR UPDATE` is released,
    // which happens after the flag above is set — so this being true is proof
    // the withdrawal genuinely waited out the hold rather than racing past it.
    expect(holderCommitting).toBe(true);

    await holder;

    expect(lockSets).toEqual([[classCId]]);

    const entry = await prisma.waitlistEntry.findFirstOrThrow({
      where: { classId: classCId, studentId: studentSId },
    });
    expect(entry.status).toBe('promoted');
  }, 20_000);
});
