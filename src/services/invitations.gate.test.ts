...
// Keeps `invitations.ts`'s logger quiet. It imports `@/lib/log`, so the
// specifier here must match that one — the same constraint `api-utils.test.ts`
// documents for its own mock.
vi.mock('@/lib/log', () => ({
 ...
 * student has withheld it, claimed or not. So this gate reads the same flag.
 *
...
describe('inviteContact — the visibility gate on ALREADY_LINKED (#412, #419)', () => {
   ...
   * test wants. `seedUnclaimedLinked` below is its unclaimed twin, and the
   * gate answers the two alike.
   *
   * The `account` is not optional decoration — `Student_claim_link_check`
  ...
  /**
   * A linked student who has never signed in, with the privacy row this test
   * wants. No `Account` and `claimedAt` left unset —
   * `Student_claim_link_check` forbids any other pairing of the two.
   */
  async function seedUnclaimedLinked(
    label: string,
    privacy: { shareEmail: boolean } | null,
  ): Promise<string> {
    const email = `gate-unclaimed-${label}-${suffix}@test.local`;
    const student = await prisma.student.create({
      data: {
        firstName: 'Gate', lastName: `Unclaimed ${label}`, email,
  ...
  it('does not answer ALREADY_LINKED for a linked, unclaimed student with no privacy row', async () => {
    const email = await seedUnclaimedLinked('unshared', null);

    const result = await inviteContact(prisma, {
      teacherId, email, firstName: 'Already', lastName: 'Mine',
    });

    // The same fall-through a claimed, unshared student gets: being unclaimed
    // releases nothing the privacy row does not.
    if (!result.ok) throw new Error(`expected a fall-through invite, got ${result.reason}`);
    expect(result.value.delivered).toBe(false);
    const row = await prisma.invitation.findUniqueOrThrow({
      where: { teacherId_email: { teacherId, email } },
  ...
  it('answers ALREADY_LINKED for a linked, unclaimed student who HAS shared their email', async () => {
    const email = await seedUnclaimedLinked('shared', { shareEmail: true });
  ...
  it('still invites an UNCLAIMED student who is not on this teacher\'s roster', async () => {
    // An unclaimed student off this roster is invited like anyone else —
    // being on the platform without having signed in tells this teacher
    // nothing.
   ...
   * roster'` above, which has a `Student` row and its own test.
   *
