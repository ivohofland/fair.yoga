...
- Adding a test that calls a service sweep taking no scope argument? It goes in `SWEEP_TESTS`, not the parallel `unit` pool, and asserts on its results the way `docs/test-database.md` §2 describes.
