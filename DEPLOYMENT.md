...
runs three sweeps and its **status is the verdict**: 200 only when every sweep
...
all three outcomes either way, so `data.auth.ok`, `data.waitlistRetention.ok`,
and `data.timezoneAudit.ok` say which one failed. Without `--fail`, `curl`
