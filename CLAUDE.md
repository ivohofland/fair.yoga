...
- A class completes 15 minutes after its end (`FINISH_GRACE_MINUTES`, `src/lib/finish-window.ts`), or earlier when the teacher finishes it, which they may from 15 minutes before the end but not before its start. Completion is when prices are calculated and payment requests go out.
...
- **A template is a stamp, not a live link** (#194, #284): editing one changes nothing that already exists — not the day, time, room, rates or capacity of any generated class, ever. The edit answers with the first week the new schedule can reach — or, for a paused or archived template, with that state instead, because the sweep skips those and no week could be named honestly; the hourly sweep is what lays a reachable one down. **One rule, and since #284 one generator**: `generateEntriesForRule` (`entry-generation.ts`) runs for either family under a `GeneratorFamily` descriptor, so the week key and the skip reasons are the same code on both sides — and `probeFirstEffectiveWeek`, in the same module, is likewise the one answer to which week an edit first reaches. A template moved Tuesday→Thursday therefore leaves its four Tuesdays standing and fills no week it already holds, whichever family it belongs to; the teacher cancels the Tuesdays they do not want, individually
...
- **One teacher, one slot, across both families** (#296, #298, #327): a
  ...
  own economics and reach their teacher through their parent. The generator
  pre-checks and skips (`blocked_by_overlap`); ten write endpoints across eight
...
3. Email fallback (unread after 30 min — sooner when the linked class starts within 2 h, and on the next sweep regardless of either for a waitlist promotion; students can opt out of optional messages, essential booking messages always email)
...
- `Account` owns auth (email identity, sessions, passkeys); `Teacher`/`Student` are optionally-linked profiles — one login serves both hats, teacher pages require a teacher profile and student pages a student one. A teacher may not link a student unilaterally: adding a CRM contact creates only an `Invitation`, and the `TeacherStudent` link forms only once the invitee accepts it (or books a class) — nothing creates an unclaimed `Student` row any more, though pre-existing unclaimed rows (created before this rule) still claim their account on first sign-in. A student who says no leaves a `TeacherBlock`, which keeps that teacher from re-adding them — written by `unlinkTeacher` after a link and by `declineInvitation` on a plain decline. The declined `Invitation` row is what makes a re-invite answer `DECLINED`, but it is not the refusal: a refusal is a row, never a key derived from an address the subject's own erasure rewrites (#522, stated with its check in `docs/data-model.md`)
...
| `docs/supply-chain.md` | Where this repo installs dependencies and why every path is `pnpm install --frozen-lockfile`; what is enforced rather than merely correct; which advisories currently stand and why CI's advisory audit step does not block on them; and the supply-chain controls not yet in place (#531) |
