...
- A class completes 15 minutes after its end (`FINISH_GRACE_MINUTES`, `src/lib/finish-window.ts`), or earlier when the teacher finishes it, which they may from 15 minutes before the end. Completion is when prices are calculated and payment requests go out.
